...
- GPU-персонализатор является отдельным Python-worker. Обычная установка ему не нужна;
  пути к моделям и enrollment передаются явно.
- Speaker-gate отвечает за решение о целевом дикторе. Это не voice cloning. Он может
  помочь оставить целевую речь и снизить чужую, но не гарантирует удаление
  каждого голоса рядом.

## Что может сломаться отдельно
