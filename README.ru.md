...
Дополнительные документы: [`CHANGELOG.ru.md`](CHANGELOG.ru.md) ·
...
- **Phrase-trigger** — шуточный эксперимент с заменой распознанной фразы.
...
| Phrase-trigger | Настраиваемый буфер задержки | CPU или CUDA | Намеренно несерьёзные мем-эксперименты |

Персонализация и phrase-trigger — необязательные слои. Если их зависимости или
чекпойнты недоступны, обычный DPDFNet-микрофон продолжает работать.

## Как дорабатывались модели
...
записях целевого диктора. Распознаватель фразы — отдельный небольшой
эксперимент, обученный на положительных и отрицательных примерах.
...
[`docs/MODEL_CARD.ru.md`](docs/MODEL_CARD.ru.md). По умолчанию он не включён.
...
Именно поэтому персональный разделитель и phrase-trigger остаются
экспериментальными. Основной HushMic должен продолжать работать, даже если
необязательная модель отсутствует, сломалась или не подходит для конкретной
комнаты.
...
Модуль замены фразы сделан намеренно несерьёзно, чисто ради мема: при распознавании
целевой фразы он может подменить её заранее записанным звуком. Это не серьёзный
голосовой ассистент, не модерация и не функция для реального использования.

Пример: [иди нахуй — демонстрация](examples/idi-nakhuy-demo.ogg).
...
Если встроенный плеер GitHub не отобразится, нажми на ссылку выше — файл можно
скачать и прослушать локально.
