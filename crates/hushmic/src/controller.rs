  ...
  {{ name = libpipewire-module-filter-chain
    ...
    args = {{
      ...
      capture.props = {{
        ...
        node.passive   = true
        # Keep the real-time deadline aligned with the DPDFNet hop.  This
        # deliberately overrides a smaller global maximum only while the
        # HushMic capture stream is active; 128-sample low-latency graphs do
        # not leave enough time for the large GPU model to finish one hop.
        node.force-quantum = {process_quantum}
        audio.rate     = 48000
        audio.channels = 1
        audio.position = [ MONO ]
{target}      }}
...
"#,
        ...
        links = links,
        process_quantum = PROCESS_QUANTUM_SAMPLES,
    )
}

/// PipeWire graph quantum while HushMic is active.  DPDFNet consumes exactly
/// one 480-sample hop at 48 kHz, so matching the graph quantum gives each GPU
/// inference its full 10 ms deadline without changing the user's idle/default
/// low-latency setting.
pub const PROCESS_QUANTUM_SAMPLES: u32 = 480;
